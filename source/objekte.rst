...
Jede einzelne Person wird im Programm durch ein *Objekt* repräsentiert. 
Das Objekt kann somit eine vereinfachte Kopie einer Person aus der realen Welt
darstellen. Jede Person ist auf seine Art einzigartig und besitzt
gewisse Eigenschaften (z.B. Name, Vorname, Geburtsdatum und Körpergewicht).
Diese Eigenschaften werden in der *Klasse* festgelegt, dass mit dem Keyword
:py:keyword:`class` gekennzeichnet ist. 
In einem Programm kann das nun folgendermassen aussehen:

.. literalinclude:: code/oop/einf_klasse.py
   :linenos:

Nun ist man in der Lage *Objekte* der Klasse :py:func:`Person()` herzustellen.
Der Befehl

>>> p1 = Person()

erstellt eine Person ohne Identität (ohne Name, Vorname etc).

>>> p1
<__main__.Person object at 0x323ded0>

Man sieht das ``p1`` eine Instanz der Klasse :py:func:`Person` ist. 
Möchte man den z.B. den ``namen`` der Person ``p1`` wissen,
so kann dieser mittels ``p1.name`` abgefragt werden.

>>> print(p1.name, p1.vorname, p1.geb_datum, p1.gewicht)
None None None None

Jetzt wäre es doch schön, wenn unserer Person ``p1`` eine Identität bekäme.
...
>>> p1.gewicht = "73.5 kg"
>>> print(p1.name, p1.vorname, p1.geb_datum, p1.gewicht)
Müller Kurt 03.02.01 73.5 kg


.. note:: Die Variablen ``name``, ``vorname``, ``geb_datum`` und ``gewicht``
	  charakterisieren die Objekte der Klasse :py:class:`Person`
	  und werden **Instanzvariablen** des entsprechenden Objektes genannt.
	  Später werden wir auch noch **Klassenvariablen** sehen, 
          welche nicht ein Objekt, sondern die Klasse beschreibt.


Die :py:func:`__init__()` Methode
=================================
Von oben ist eigentlich klar sein, dass es vielleicht keinen Sinn macht,
Personen ohne Identität herzustellen. Es wäre also nur logisch, 
wenn man bei der Herstellung eines ``Objekts`` die Instanzvariablen von Anfang 
an definieren könnte (in unserem Beispiel würde das heissen, dass wir der Person
gleich bei der Herstellung eine Identität geben).
Dies kann man machen, wenn in der Klasse die Funktion :py:func:`__init__()` 
existiert. Die gewünschte Belegung der Instanzvariblen können 
...
Auf diese Weise kann keine Person ohne Identität erstellt werden.
...
.. note:: Das Argument ``self`` steht als Vertreter für das Objekt.
...
der jeweiligen Personen, indem sie sich selber vorstellen.
...
Eigenschaften und stellt sich dementsprechend auch anders vor.
...
.. note:: Die Variablen der Klasse :py:class:`Person` (``name``, ``vorname``,
	  ``geb_datum`` und ``gewicht``) sowie die Klassenmethoden 
	  (:py:func:`vorstellen` und  :py:func:`abnehmen`) sind nur Objekten
	  derselben Klasse :py:class:`Person` vorbehalten.
	  Eine Eingabe wie
